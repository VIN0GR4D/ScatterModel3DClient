...
void ScatterPlot3DWidget::setScatteringData(const QVector<double> &absEout, const QVector<double> &normEout) {
    ...
    // Проверяем, совпадает ли размер данных с количеством треугольников
    if (this->absEout.size() != indices.size() || this->normEout.size() != indices.size()) {
        qDebug() << "Предупреждение: Размеры absEout или normEout не совпадают с количеством треугольников.";
        qDebug() << "Размер absEout:" << this->absEout.size() << "Размер normEout:" << this->normEout.size() << "Количество треугольников:" << indices.size();
        return;
    }
...
void ScatterPlot3DWidget::drawScattering() {
    ...
    for (int t = 0; t < indices.size(); ++t) {
        const QVector<int> &index = indices[t];
        if (index.size() == 3) {
            // Получаем значение для текущего треугольника
            double value = absEout[t];
            double normalizedVal = (value - minAbsEout) / (maxAbsEout - minAbsEout);
